...
public static class Lab1
{
    ...
    {
        var lines = inputText.ReplaceLineEndings().Split(Environment.NewLine)
            .AsEnumerable()
            .GetEnumerator();
        var input = Read(lines);
        var numberPermutations = CountPermutations(input.N, input.K);
        return numberPermutations.ToString();
    }


    private static Lab1UserInput Read(IEnumerator<string> lines)
    {
        if (!lines.MoveNext())
        {
